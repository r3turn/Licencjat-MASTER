# utils/gru_utils.py - GRU dla prognozowania zmienności
#
# GRU (Gated Recurrent Unit) - uproszczona wersja LSTM.
# Ma 2 bramki (reset, update) zamiast 3 (forget, input, output).
# Zalety: szybsze trenowanie, mniej parametrów, często podobne wyniki.
#
# Zawiera:
# - Architekturę modelu GRU
...
class GRUVolatility(nn.Module):
    """
    GRU do prognozowania zmienności (wariancji).

    Architektura:
    - GRU layer(s) przetwarzający sekwencję zwrotów
    ...
    - Mniej parametrów -> szybsze trenowanie
    - Brak osobnego cell state (tylko hidden state)
    """
    ...
    def __init__(self, input_size=1, hidden_size=32, num_layers=1, dropout=0.1):
        ...
        # GRU - batch_first=True oznacza input shape (batch, seq, features)
        self.gru = nn.GRU(
    ...
    def forward(self, x):
        ...
        # GRU zwraca tylko hidden state, nie ma cell state jak LSTM
...
def walk_forward_gru(returns_series, window_size, initial_train_size,
    ...
    hidden_size, num_layers, dropout : hiperparametry GRU
    ...
    # Progress bar
    pbar = tqdm(range(initial_train_size, n), desc="Walk-forward GRU")
