...
class GRUVolatility(nn.Module):
    """
    GRU do prognozowania zmienności (wariancji warunkowej).

    GRU vs LSTM: dwie bramki (reset, update) zamiast trzech (forget, input, output).
    Brak osobnego cell state — mniej parametrów, szybsze trenowanie.

    Architektura:
    - Warstwa GRU przetwarzająca sekwencję stóp zwrotu
...
def train_gru(model, X_train, y_train, X_val, y_val,
    ...
    """
    Trenuj GRU z early stopping na zbiorze walidacyjnym.
...
def static_split_gru(returns_series, window_size, train_ratio=0.8, val_ratio=0.1,
                     hidden_size=32, num_layers=1, dropout=0.1,
                     epochs=100, batch_size=32, learning_rate=0.001,
                     patience=15, device='cpu', seed=42):
    """
    Ewaluacja GRU z podziałem 80/10/10 i sliding window na zbiorze testowym.
    ...
    hidden_size, num_layers, dropout : architektura GRU
    ...
    model = GRUVolatility(input_size=1, hidden_size=hidden_size,
                          num_layers=num_layers, dropout=dropout)
    model, _, _ = train_gru(model, X_train, y_train, X_val, y_val,
