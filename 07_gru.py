...
for ticker in TICKERS:
    ...
    plt.tight_layout()
    plt.savefig(f"charts/07_gru/scatter_{ticker}.png", dpi=150)
    plt.close()
...
for ax, metric, title in zip(axes, metrics_to_plot, titles):
    ax.bar(results_df['ticker'], results_df[metric], color='teal', edgecolor='black')
    ax.set_ylabel(title)
...
plt.tight_layout()
plt.savefig("charts/07_gru/metrics_comparison.png", dpi=150)
plt.close()
